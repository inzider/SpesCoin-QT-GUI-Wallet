...
//
// Spes is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Spes is distributed in the hope that it will be useful,
...
const char DEFAULT_WALLET_FILE_NAME[] = "spes.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.myspes.org:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("speswallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("speswallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("speswallet.desktop");
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.spes.speswallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=SPES Wallet\n");
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\spes", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:spes");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\spes\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\spes\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("speswallet.cfg"));
