...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("spescoinwallet.lock"));
  QUrl paymentUrl = QUrl::fromUserInput(arguments().last());
  if (paymentUrl.scheme() != SPES_URI_SCHEME_NAME) {
    paymentUrl = QUrl();
  }
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] SPES wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "SPES wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    if (m_splash != nullptr) {
      m_splash->show();
      m_splash->showMessage(QObject::tr("v3.0 - Loading blockchain..."), Qt::AlignLeft | Qt::AlignBottom, Qt::white);
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have spescoind with non-default RPC port, you should terminate it and relaunch LuKaWallet\n"
      "or\n"
